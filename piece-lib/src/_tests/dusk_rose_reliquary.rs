...
fn exiles_until_leaves_battlefield() -> anyhow::Result<()> {
    ...
    let card = CardId::upload(&mut db, &cards, player1.clone(), "Dusk Rose Reliquary");
    let card2 = CardId::upload(&mut db, &cards, player1.clone(), "Alpine Grizzly");
    let card3 = CardId::upload(&mut db, &cards, player2.clone(), "Abzan Banner");
    let card4 = CardId::upload(&mut db, &cards, player1.clone(), "Alpine Grizzly");
    let card5 = CardId::upload(&mut db, &cards, player1.clone(), "Deconstruction Hammer");
