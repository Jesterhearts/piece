...
fn spawns_bats() -> anyhow::Result<()> {
    ...
    let mut results = Battlefield::activate_ability(&mut db, &None, player, cave1, 0);
    let result = results.resolve(&mut db, None);
    assert_eq!(result, ResolutionResult::Complete);
    let mut results = Battlefield::activate_ability(&mut db, &None, player, cave2, 0);
    let result = results.resolve(&mut db, None);
    assert_eq!(result, ResolutionResult::Complete);
    let mut results = Battlefield::activate_ability(&mut db, &None, player, cave3, 0);
    let result = results.resolve(&mut db, None);
    assert_eq!(result, ResolutionResult::Complete);
